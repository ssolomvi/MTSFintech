...
public class CreateAnimalServiceImpl implements CreateAnimalService {
    ...
    public CreateAnimalServiceImpl(AnimalFactory animalFactory) {
        if (animalFactory == null) {
            throw new AnimalConfigurationStarterIllegalArgument("CreateAnimalServiceImpl:: param 'animalFactory' is null");
        }
    ...
    @Override
    public void setAnimalType(AnimalType setAnimalType) {
        if (setAnimalType == null) {
            throw new AnimalConfigurationStarterIllegalArgument("CreateAnimalServiceImpl::setAnimalType: param " +
                    "animalType should not be null");
        }
        this.animalType = setAnimalType;
    }
...
//        System.out.println("It's override method in class CreateAnimalServiceImpl");
    ...
    public Map<String, List<Animal>> createRandomAnimals(int n) {
        if (n <= 0) {
            throw new AnimalConfigurationStarterIllegalArgument("CreateAnimalServiceImpl::createRandomAnimals:" +
                    "param n must be >= 1");
        }
        Map<String, List<Animal>> animals = new HashMap<>(AnimalType.values().length);
